...
<template>
    <div class="title">
        <span class="type">
            ...
            <span v-else-if="question.type === 'multipleChoice'">
                [多选题]
            </span>
            <span v-else-if="question.type === 'fillInTheBlanks'">
                [填空题]
            </span>
            <span v-else-if="question.type === 'subjective'">
                [主观题]
            </span>
    ...
    <ul class="option-list" v-if="question.type === 'singleChoice' || question.type === 'multipleChoice'">
        <li class="option" v-for="(option, optionIndex) in question.options" :key="optionIndex"
            @click="selectOption(question, option)" :class="{ selected: option.select }">
    ...
    <input type="txet" required class="input-text" v-model="question.answer"
        v-if="question.type === 'fillInTheBlanks'" />
    <div :class="{ resize: question.type === 'subjective' }">
        <textarea required class="input-textarea" v-model="question.answer" placeholder="请在此处作答"
            v-if="question.type === 'subjective'"></textarea>
    </div>
</template>
